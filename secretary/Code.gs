...
function handleScheduledPost(text, chatId, senderId, replyToken, owner) {
  ...
  // งานที่ต้อง "แต่ง/เรียบเรียงข้อความ" ก่อน → ปล่อยให้สมอง AI จัดการ (จะแต่งเสร็จแล้วตั้งเวลาผ่านบล็อก SCHEDULE เอง)
  // ถ้าให้ regex เก็บตรงนี้ เนื้อหาจะถูกบันทึกดิบๆ ตามที่พิมพ์ ไม่ได้แต่งอย่างที่ขอ
  if (/ช่วยคิด|ช่วยแต่ง|คิดคำ|แต่งคำ|เรียบเรียง|ภาษาของคุณ|น้ำเสียง|สำนวน|โทน|ใจความ(?:ว่า)?|ประมาณว่า|ทำนองว่า/.test(t)) return false;
